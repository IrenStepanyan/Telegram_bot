...
class TelegramBotAnalytics:
    ...
    @sync_to_async
    def log_interaction_sync(self, user, interaction_type, **kwargs):	
    ...
    async def log_interaction(self, user: TelegramUser, interaction_type: str, **kwargs):
        await self.log_interaction_sync(user, interaction_type, **kwargs)
...
Your data is used only for generating your personal stats.
"""
...
"""
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await query.message.reply_text("Your selection has been processed!")

    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        user = await self.get_or_create_user(update)
        await self.log_interaction(user, 'message', message_text=update.message.text[:500])
